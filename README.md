...
Ключевые переменные окружения (полный список — `.env.example`):

| Переменная | Назначение |
|------------|------------|
| `JWT_SECRET` | Секрет JWT (≥32 символов); в dev может генерироваться автоматически |
| `AUTH_ADMIN` | Администратор: `email\|имя\|api_key` |
| `MAX_UPLOAD_MB` | Лимит размера загружаемого файла |
| `CORS_ORIGINS` | Разрешённые origin для браузера |
| `PIPELINE_WORKERS` | Параллельные воркеры обработки документов |
| `GRAPH_FETCH_LIMIT` | Лимит узлов при загрузке графа в UI |
